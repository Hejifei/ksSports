<template>
  <div class="hello">
    <div class="listtitle">群体活动</div>
    <div class="content">
      <div class="flex-3"  v-for="(item,index) in list" :key="index">
        <router-link :to="{name:'groupgame-manage',query:{typeId:item.typeId}}" tag="div" class="img"  :style="{background:`url('${item.matchBackImg}') no-repeat center center`,'backgroundSize':'cover'}">
          <!-- <div> -->
...
export default {
  ...
  methods:{
    async init(){
      getLeagueClassification({matchType:2}).then(res=>{
        this.list=res.data
      })
    }
  },
