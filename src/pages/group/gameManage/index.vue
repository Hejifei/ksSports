  ...
  <div class="hello">
    ...
    <div class="listSearchC">
      ...
      <el-row>
        <el-col :span="12">
          <div class="listsecBox">
            <router-link :to='{name:"groupgame-add"}'  class="addBtn">
            </router-link>
            <!-- <a  class="addBtn"></a> -->
          </div>
          <div class="listsecBox">
  ...
  methods: {
    handleScore(val){
      if(val.status===4 || val.status===5){
        this.$router.push({name:'groupgame-score',query:{id:val.matchId,matchName:val.matchName}})
      }
    },
    handleOrder(val){
    ...
    listedit() {
      if(this.refereeId.length > 1){
        ...
        this.$router.push({name:'groupgame-edit',query:{'id':this.refereeId[0]}})
      }else{
        this.$message({
            type: 'warning',
            message: '您还没有勾选要编辑的项！'
