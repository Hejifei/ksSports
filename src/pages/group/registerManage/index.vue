      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <router-link type="text" class="el-button el-button--text" :to='{name:"groupapproval-enroll",query:{id:scope.row.enrollId,type:"view",matchId:scope.row.matchId}}' >{{scope.row.clubName}}</router-link>
          </template>
        </el-table-column>
        <el-table-column
          prop="projectName"
...
export default {
  ...
  methods: {
    approval(val){
      if(val.status==1){
        this.$router.push({name:'groupapproval-enroll',query:{id:val.enrollId,matchId:val.matchId}})
      }
    },
    exportout(){
